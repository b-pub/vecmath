                          ...
                          Matrix3<fptype> const& matb)
{
    ...
    // Unrolling this loop does not make it faster
    for (int i=0; i<4; i++)
    {
        fptype (&r)[4] = result.m_m[i];

        r[0] = (a[i][0]*b[0][0] + a[i][1]*b[1][0] + a[i][2]*b[2][0] + a[i][3]*b[3][0]);
        r[1] = (a[i][0]*b[0][1] + a[i][1]*b[1][1] + a[i][2]*b[2][1] + a[i][3]*b[3][1]);
        r[2] = (a[i][0]*b[0][2] + a[i][1]*b[1][2] + a[i][2]*b[2][2] + a[i][3]*b[3][2]);
        r[3] = (a[i][0]*b[0][3] + a[i][1]*b[1][3] + a[i][2]*b[2][3] + a[i][3]*b[3][3]);
    }
